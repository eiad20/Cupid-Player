...
async function ytDlpSearch(title, artist) {
  const { stdout } = await execFileAsync(getYtDlpPath(), [
    ...
    '--cookies-from-browser', 'chrome', // <-- Add this line here
    '-f', 'bestaudio[ext=m4a]/bestaudio',
    '--no-playlist',
    '--no-warnings',
    '--print', '%(id)s',
...
ipcMain.handle('get-local-playlist', async () => {
  const AUDIO_EXTS = new Set(['.mp3', '.m4a', '.aac', '.flac', '.wav', '.ogg', '.opus']);
  const dir = userAudioDir();

  // Load playlist.json for user-supplied metadata (optional)
  let metaMap = new Map();
  try {
    ...
    if (Array.isArray(parsed)) {
      for (const entry of parsed) {
        if (entry.file) metaMap.set(entry.file, entry);
      }
    }
  } catch (err) {
  ...
  // Scan audio folder for all supported files
  let files = [];
  try {
    const entries = await fs.promises.readdir(dir);
    files = entries.filter((f) => AUDIO_EXTS.has(path.extname(f).toLowerCase()));
  } catch {
    return [];
  }

  // Merge: use playlist.json metadata if available, otherwise use filename as title
  return files.map((filename) => {
    if (metaMap.has(filename)) return metaMap.get(filename);
    const name = path.basename(filename, path.extname(filename));
    return { file: filename, title: name };
  });
});
